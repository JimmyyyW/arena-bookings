...
class BookingController(
    ...
    @PostMapping("/bookingslp")
    fun createBooking(@RequestBody httpBookingRequest: HttpBookingLpRequest): ResponseEntity<Any> {

        val booking = BookingLp(
            bookingLpId = null,
            horse = horseService.findHorseById(httpBookingRequest.horseId),
            startTime = httpBookingRequest.startTime,
            endTime = httpBookingRequest.endTime
        ...
        return when(val bookingOrError = bookingService.createBookingLp(booking)) {
            is Either.Left -> ResponseEntity<Any>(bookingOrError.value, HttpStatus.CREATED)
            is Either.Right -> ResponseEntity<Any>(ErrorMessage(bookingOrError.value.message), HttpStatus.NOT_ACCEPTABLE)
        }
    }
    ...
    @GetMapping("/bookingslp")
    fun getBookingsLp(): List<HttpBookingLpResponse> {
        return bookingService.findAllBookingsLp()
            .map { HttpBookingLpResponse(
                    it,
                    it.horse.customer.customerId
            ) }
    }
...
data class HttpBookingLpResponse(
    val bookingLp: BookingLp,
    val customerId: Int?
)
...
data class HttpBookingLpRequest(
    val horseId: Int,
